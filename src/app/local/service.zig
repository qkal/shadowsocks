...
pub const RunningLocal = struct {
    ...
    pub fn stop(self: *RunningLocal) void {
        ...
        if (!state.stop_requested.load(.acquire)) {
            ...
            const local = state.config.local orelse unreachable;
            wakeListener(local.bind_host, local.bind_port);
        }
...
pub fn runLocal(config: RuntimeConfig) !RunningLocal {
    ...
    const local = state.config.local orelse return error.MissingLocalConfig;
    var listener = try tcp.listenIp(local.bind_host, local.bind_port);
